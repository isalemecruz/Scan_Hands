...
foreach ($campos as $campo) {
    if (!empty($_GET[$campo])) {
        if ($campo === 'turma') {
            $conditions[] = "t.nome LIKE ?";
        } else {
            $conditions[] = "u.$campo LIKE ?";
        }
        $params[] = "%" . $_GET[$campo] . "%";
    }
}
...
// Query com filtros e GROUP_CONCAT
$sql = "
SELECT 
  p.id_professor,
  ...
  u.cpf,
  u.telefone,
  COALESCE(GROUP_CONCAT(DISTINCT t.nome ORDER BY t.id_turma SEPARATOR ', '), 'sem turma') AS turmas
FROM professor p
JOIN usuario u ON u.id_usuario = p.id_usuario
LEFT JOIN professor_turma pt ON pt.id_professor = p.id_professor
LEFT JOIN turma t ON t.id_turma = pt.id_turma
WHERE $whereClause
...
<div class="sidebar" style="margin-right: 50px;">
  <div class="sidebar-top">
      ...
      <button style="font-size: 12px;" onclick="window.location.href='perfilSecretaria.php'">Ver Perfil</button>
      <nav>
...
<div class="main">
  <div class="card-container">
    <div class="header-buttons">
      <button id="filterToggle" class="btn btn-outline-secondary">⚙️ Filtrar Professor</button>
      <button class="btn btn-outline-secondary" onclick="window.location.href='adicionaProfessor.php'">➕ Adicionar Professor</button>
        ...
        <?php if ($result && $result->num_rows > 0): ?>
          <?php while($row = $result->fetch_assoc()): ?>
            <tr>
              ...
              <td><?= htmlspecialchars($row['turmas']) ?></td>
              <td style="position: relative;">
                ...
                <div class="options-card">
                  ...
                  <a href="excluirProfessor.php?cpf=<?= urlencode($row["cpf"]); ?>">Excluir professor</a>
                  <a href="alterarProfessor.php?cpf=<?= urlencode($row["cpf"]); ?>">Alterar professor</a>
                </div>
              </td>
            </tr>
          <?php endwhile; ?>
        <?php else: ?>
          <tr><td colspan="6">Nenhum professor encontrado.</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
...
<script>
  ...
  document.querySelectorAll('.open-search').forEach(btn => {
    btn.addEventListener('click', function(e) {
      ...
      searchInput.placeholder = `Digite o ${field === 'cpf' ? 'CPF' : field === 'telefone' ? 'telefone' : field} do Professor`;
      searchInput.value = '';
