...
/**
 * Mostrar erros em tela (útil enquanto ajusta).
 * Em produção, você pode comentar as 2 linhas abaixo.
 */
...
$conn->set_charset('utf8mb4'); // alinha com suas collations

// Carregar turmas para o <select>
$turmas = [];
$resTurmas = $conn->query("SELECT nome FROM turma ORDER BY nome");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validações básicas
    if (!$nome || !$turmaNome || !$emailNovo || !$telefone || !$senhaUsuario || !$cpf) {
        echo "<script>alert('Preencha todos os campos.');</script>";
    } elseif (!isset($_FILES['foto']) || $_FILES['foto']['error'] !== UPLOAD_ERR_OK) {
        echo "<script>alert('Selecione uma foto válida.');</script>";
    } else {
        try {
            // Lê a foto (como string)
            $fotoBinario = file_get_contents($_FILES['foto']['tmp_name']);

            // Inicia transação
            $conn->begin_transaction();

            // 1) Inserir usuário (senha em texto puro, como solicitado)
            $stmt = $conn->prepare("
                INSERT INTO usuario (foto_perfil, nome, email, cpf, telefone, senha, tipo)
                VALUES (?, ?, ?, ?, ?, ?, 'aluno')
            ");
            // use 's' para enviar o binário como string (BLOB aceita string binária)
            $stmt->bind_param("ssssss", $fotoBinario, $nome, $emailNovo, $cpf, $telefone, $senhaUsuario);
            $stmt->send_long_data(0, $fotoBinario); // opcional; pode remover se preferir
            $stmt->execute();
            $novoUsuarioId = $stmt->insert_id;
            $stmt->close();

            // 2) Inserir aluno (agora só id_usuario, pois a turma é via relação)
            $stmtAluno = $conn->prepare("INSERT INTO aluno (id_usuario) VALUES (?)");
            $stmtAluno->bind_param("i", $novoUsuarioId);
            $stmtAluno->execute();
            $novoAlunoId = $stmtAluno->insert_id;
            $stmtAluno->close();

            // 3) Encontrar id_turma pelo nome selecionado
            $stmtTurma = $conn->prepare("SELECT id_turma FROM turma WHERE nome = ?");
            $stmtTurma->bind_param("s", $turmaNome);
            $stmtTurma->execute();
            $resTurma = $stmtTurma->get_result();
            if ($resTurma->num_rows === 0) {
                // Se a turma não existir, você pode optar por criar automaticamente:
                // $stmtCreate = $conn->prepare("INSERT INTO turma (nome, qtd_aluno, qtd_atividade, qtd_aula, qtd_prof) VALUES (?,0,0,0,0)");
                // $stmtCreate->bind_param("s", $turmaNome);
                // $stmtCreate->execute();
                // $idTurma = $stmtCreate->insert_id;
                // $stmtCreate->close();
                // Ou lançar um erro:
                throw new Exception("Turma selecionada não existe no banco: '$turmaNome'.");
            }
            $idTurma = $resTurma->fetch_assoc()['id_turma'];
            $stmtTurma->close();

            // 4) Criar vínculo aluno_turma
            $stmtRel = $conn->prepare("
                INSERT INTO aluno_turma (id_aluno, id_turma, data_matricula)
                VALUES (?, ?, CURDATE())
            ");
            $stmtRel->bind_param("ii", $novoAlunoId, $idTurma);
            $stmtRel->execute();
            $stmtRel->close();

            // (Opcional) Atualizar contadores na turma, se você usa esses campos
            // $conn->query("UPDATE turma SET qtd_aluno = qtd_aluno + 1 WHERE id_turma = ".(int)$idTurma);
            ...
            // Reverte se algo deu errado
            if ($conn->errno === 0) { // se não for erro mysqli, ainda assim tenta rollback
                try { $conn->rollback(); } catch (Throwable $ignore) {}
            } else {
                try { $conn->rollback(); } catch (Throwable $ignore) {}
            }

            // Mostra erro detalhado para você depurar
            // Em produção, troque por um log e uma mensagem amigável
            echo "<pre style='white-space:pre-wrap;font-family:monospace;color:#b00;background:#fee;padding:12px;border-radius:8px;'>
    ...
    function previewImage(event) {
        ...
        reader.onload = function () {
            document.getElementById('profileImage').innerHTML =
                `<img src="${reader.result}" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;">`;
        };
