...
// Query agora com JOIN entre usuario, aluno, aluno_turma e turma
$sql = "
    SELECT 
        u.nome, 
        u.email, 
        u.cpf, 
        u.telefone, 
        GROUP_CONCAT(DISTINCT t.nome SEPARATOR ', ') AS turma
    FROM usuario u
    INNER JOIN aluno a ON u.id_usuario = a.id_usuario
    LEFT JOIN aluno_turma at ON a.id_aluno = at.id_aluno
        ...
        <?php if ($result && $result->num_rows > 0): ?>
          <?php while($row = $result->fetch_assoc()): ?>
            <tr>
              ...
              <td><?php echo htmlspecialchars($row["turma"] ?? 'Sem turma'); ?></td>
